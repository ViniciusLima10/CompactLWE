    ...
    for npair in range(pairs):
        ...
        for _ in range(trials):
            ...
            if mu == mucand:
                succnow += 1
        succ += succnow
        print(f"Key pair {npair} complete. Success rate: {succnow}/{trials}.")
    print(f"Successful recoveries: {succ}/{trials * pairs} ({100 * succ / trials / pairs}%).")
    print(f"Average time: {tottime / trials / pairs} seconds.")

# Run the test
testsubsetsumdecrypt(trials=100, pairs=1)
